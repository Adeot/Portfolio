...
Queremos estudiar un sistema de muchas partículas moviéndose en una caja. Pensemos, por ejemplo, en un gas, cuyas moléculas se repelen entre sí y chocan contra las paredes. Para simplificar, trabajaremos con una caja en el plano.
...
md"""###### Ejercicio 1:
Primero implementamos una función que genere un dato inicial aleatorio. Es decir: que reciba un parámetro $n$ (la cantidad de partículas a generar) y devuelva alguna estructura de datos adecuada que contenga las posiciones iniciales, tomadas al azar en el cuadrado $[0,L]\times[0,L]$ y las velocidades iniciales, también tomadas al azar. 

Dado que las velocidades pueden influir en la dinámica de las simulaciones, creamos una función que recibe un parámetro de escala `V` y genera para cada partícula un ángulo al azar `α∈[0,2π)` y un valor al azar `v∈[0,V]`, los cuales definen las coordenadas de la velocidad. De esta manera se obtienen vectores de velocidad distribuidos uniformemente en el círculo de radio `V`, y es posible modificar el valor de `V` de una simulación a otra. """

# ╔═╡ ff6ebf08-69b3-4d3c-82b4-af70f12ddd30
# dato inicial
function condiciones_iniciales(n, V)
	uₒ = zeros(4, n) # matriz de 4xn
	for i in 1:n
		α = 2*π*rand()
		v = V*rand()
		uₒ[1,i] = L*rand() # px
...
Implementamos una función que define el sistema de ecuaciones. Asumimos que el vector de parámetros `p` toma la forma `p=[k₁,m]`, donde `m=[m₁,m₂,…,mₙ]` es el vector de masas.
 """

# ╔═╡ 2c1b3f12-4588-452f-aebd-27f20535f951
function norma_cuadrado(u)
	return sum(u.*u)
end
...
function gas!(du, u, p, t)
	...
	for i in 1:length(u[1,:])
		...
		for j in 1:length(u[1,:])
			...
			dij_cuad = norma_cuadrado(u[1:2,i]-u[1:2,j])
			du[3, i] = du[3, i] + 4*k₁*m[j]*(u[1, i]-u[1, j])/dij_cuad^3
			du[4, i] = du[4, i] + 4*k₁*m[j]*(u[2, i]-u[2, j])/dij_cuad^3
		end
	end
end
...
Para reconocer los choques contra las paredes utilizaremos un `DiscreteCallback`. Necesitamos:
...
Estos dos puntos deben revisarse en _todas_ las partículas. """
...
function condicion_rebote(u,t,integrator)
	for i in 1:length(u[1,:])
		if (u[1,i] < 0 || u[1,i] > L || u[2,i] < 0 || u[2,i] > L)
			return true
		end
	end
	return false
...
function rebote!(integrator)
	...
	for i in 1:length(integrator.u[1,:])
		...
		# toca alguna pared
		if integrator.u[1,i] < 0
			integrator.u[1,i] = -integrator.u[1,i] # se corrige px
			integrator.u[3,i] = -integrator.u[3,i] # se invierte vx
		elseif integrator.u[1,i] > L
			integrator.u[1,i] = -integrator.u[1,i] + 2*L # se corrige px
			integrator.u[3,i] = -integrator.u[3,i] # se invierte vx
		end

		# toca el piso o el techo
		if integrator.u[2,i] < 0
			integrator.u[2,i] = -integrator.u[2,i] # se corrige py
			integrator.u[4,i] = -integrator.u[4,i] # se invierte vy
		elseif integrator.u[2,i] > L
			integrator.u[2,i] = -integrator.u[2,i] + 2*L # se corrige py
			integrator.u[4,i] = -integrator.u[4,i] # se invierte vy
		end
...
Implementamos una función que recibe un conjunto de parámetros adecuados y ejecuta una simulación."""



# ╔═╡ 8d9e0b1a-8da0-4521-9961-e45969fe2653
function simulacion_gas(N, V, m, k, tspan)
	u₀ = condiciones_iniciales(N, V)
	p = [k, m]
	dc = DiscreteCallback(condicion_rebote, rebote!)
	prob = ODEProblem(gas!,u₀,tspan,p)
	sol = solve(prob, callback=dc, dtmax = 0.1)
end

# ╔═╡ a5fa4f83-3169-4268-b816-7b8fe2c5333a
md"""###### Ejercicio 5:

Para visualizar los resultados implementamos una función que recibe la solución de una simulación y realiza una animación. Cada cuadro de la animación contiene las líneas que delimitan la caja y un scatter plot de las posiciones de las masas en un instante determinado. 

Tomando $k_1 = 10^{-6}$ y masas iguales a 1, realizamos simulaciones con 2, 5 y 10 partículas. Experimentamos con distintas velocidades iniciales y comprobamos que las interacciones entre partículas y los choques con las paredes se realizan correctamente. 

Por último, estudiamos los casos en que las masas son variables. Para ello simulamos un ejemplo con masas aleatorias entre $0.5$ y $2.5$, graficándo los puntos con distintos tamaños, escalados de acuerdo a su masa. """

# ╔═╡ 35f58720-56f4-4cce-adab-95414062f3d0
# animación

function animacion(sol, V, m)

	N = length(sol(1)[1,:])
	ti = sol.t[1]
	tf = sol.t[end]
	
	@gif for i in ti:0.2:tf
		scatter([sol(i)[1,1]], [sol(i)[2,1]], shape = :circle, xlims=[0,L], ylims = [0,L], markersize = 5*m[1], legend = false)
		for j in 2:N
			scatter!([sol(i)[1,j]], [sol(i)[2,j]], shape = :circle, xlims=[0,L], ylims = [0,L], markersize = 5*m[j], legend = false)
		end
		plot!([0, L], [L, L], color = :black, legend = false) #techo
		plot!([L, L], [0, L], color = :black, legend = false, title = "Simulación con N = $N y Vmax = $V") #pared derecha
	end
...
sol₁ = simulacion_gas(N₁, V₁, m₁, k, tspan)
animacion(sol₁, V₁, m₁)

end

# ╔═╡ 0540091f-6ce0-4a66-83e3-a57c3675611f
begin
...
# ╔═╡ 7d6ed18b-4534-4f60-8bd2-22d555e65f11
md""" ###### Reducimos un poco la velocidad para una mejor visualización"""

# ╔═╡ cb4a79a3-26ec-45dc-9414-33418d1fa529
begin
...
# ╔═╡ 660cec21-4be3-4f0c-b0a5-df72d5e12d4e
md""" ###### Volvemos a reducir la velocidad"""
...
# ╔═╡ aafd4d35-43d9-41db-b897-ddcc026d6943
md""" ###### Ahora con masas variables entre 0.5 y 2.5, las cuales están representadas por los tamaños de las partículas."""

# ╔═╡ 29da6749-52fb-4881-88d8-556613629dac
begin

# prueba con 50 con masas entre 0.5 y 2.5

N₅ = 50
V₅ = L/10
m₅ = [2*rand()+0.5 for i in 1:N₅]
...
donde el primer término es el mismo con el que trabajamos previamente y el segundo es atractivo. Experimentamos con distintos valores de $k_2$ y de las velocidades iniciales, buscando que se formen cúmulos de partículas (pequeños o grandes)."""

# ╔═╡ 84d5d1f3-3592-470b-a75d-c210cbfd8171
md"""Con este potencial, la fuerza $F_i^j$ que la masa $j$ ejerce sobre la masa $i$ es: 

$F_i^j = -\nabla_i E^P_{ij} = 4k_1 m_i m_j \frac{(x_i,y_i)-(x_j,y_j)}{d_{ij}^6} - 3k_2 m_i m_j \frac{(x_i,y_i)-(x_j,y_j)}{d_{ij}^5},$

mientras que $F_j^i = -F_i^j$. La fuerza total sobre la partícula $i$ será la suma de las fuerzas que todas las otras partículas ejercen sobre ella. 

Realizamos simulaciones con masas iguales y con masas variables.

"""

# ╔═╡ b7f4b557-b34f-4e7f-b6f1-17fec8b9c449
# ecuaciones del modelo modificado

function gas_2!(du, u, p, t)
	k₁ = p[1]
	k₂ = p[2]
	m = p[3]
	for i in 1:length(u[1,:])
		...
		for j in 1:length(u[1,:])
			...
			dij_cuad = norma_cuadrado(u[1:2,i]-u[1:2,j])
			du[3, i] = du[3, i] + 4*k₁*m[j]*(u[1, i]-u[1, j])/dij_cuad^3 - 3*k₂*m[j]*(u[1, i]-u[1, j])/dij_cuad^(5/2) 
			du[4, i] = du[4, i] + 4*k₁*m[j]*(u[2, i]-u[2, j])/dij_cuad^3 - 3*k₂*m[j]*(u[2, i]-u[2, j])/dij_cuad^(5/2)
		end
	end
end

# ╔═╡ 44fa548e-6bea-4028-bd00-f8062c896499
function simulacion_gas_2(N, V, m, k₁, k₂, tspan)
	u₀ = condiciones_iniciales(N, V)
	p = [k₁,k₂, m]
	dc = DiscreteCallback(condicion_rebote, rebote!)
	prob = ODEProblem(gas_2!, u₀, tspan, p)
	sol = solve(prob, callback=dc, dtmax = 0.1)
end

# ╔═╡ ba11455d-0231-4e79-babd-beac78a29bd5
md""" ###### Para lograr que se aprecie el acercamiento entre las partículas, redujimos aún más la velocidad máxima. """

# ╔═╡ 4702614b-415c-449e-965e-bd0a15282eee
begin
...
sol₆ = simulacion_gas_2(N₆, V₆, m₆, k, k_2, tspan)
animacion(sol₆, V₆, m₆)

end

# ╔═╡ 88b651cf-3118-4ae5-ab5b-886270cc0269
md""" ###### Ahora con masas variables entre 0.5 y 2.5. """

# ╔═╡ f25c19a2-3a0b-418a-b69e-c8e1cfdd9d87
begin
